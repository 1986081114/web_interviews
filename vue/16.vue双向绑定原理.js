 ...
 /*   vue双向绑定原理：
       ...
       优势： 无需显示调用， 例如在vue运行数据劫持+发布订阅，直接可以通知变化并驱动视图，想react需要调用setState
              可精确得知变化数据，在setter函数内可以获得newVALUE,不需要额外的操作， 
       ...
       mvvm包含两个方面， 数据变化更新视图，试图变化更新数据。
        ...
        实现： 
           1.首先对数据进行监听： 设置一个监听器Observer，用来监听所有属性。，给对象属性添加getter 和setter，用于依赖收集和派发更新。
           2.订阅器Dep， 用来收集响应式对象的依赖关系， 对监听器Observer和订阅者Watcher统一管理，dep.sub = watcher[], 当数据发生变化， 会通知
            dep.notify()通知各个watcher。
            3.订阅者watcher，可以收到属性的变化通知并执行相应的方法， 从而更新视图，包括render wather， computed watcher， user watcher。
