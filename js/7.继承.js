...
/* 1.原型链方式 
      ...
      缺点： 子类实例不能给父类构造函数传参，即子类不能通过父类创建私有属性
             引用类型的属性被所有实例共享。           
*/
...
混合继承
  ...
  缺点： 会两次调用父类构造函数， 第一次是一次是设置子类型实例的原型的时候： new Person（）
     第二次是在次在创建子类型实例的时候：new Child("kevin", '18');因为在new我们会执行 Person.call(this, name);
  ，第二次调用产生的实例属性和方法，会覆盖第一次调用在原型上产生的属性和方法， 这里是name， className*/
...
/*
es6继承
class类通过extends和super实现继承， 子类必须在constructor方法中调用super方法，否则子类就得不到自己的this方法。
...
/* 
static: 
   凡是被static修饰的方法，都是静态方法和属性（目前还没有）， 只能被类名调用， 不能被new实例化对象调用， 同时也不能被子类继承， 换句话说
